...
//    [meetingInfoView addSubview:text];
    ...
    [self.view addSubview:profile.bgImageProfile];
...
{
    float widthViews = screenWidth * 0.5;
    ...
    UIButton *followersLabelContainerBtn = [[UIButton alloc] initWithFrame:CGRectMake(metUserFBView.frame.size.width - widthViews,
                                                                                      metUserFBView.frame.size.height - (62 + 45),
                                                                                      widthViews, 64)];
...
- (void) displayMetUserfbImgProfileForDatas:(NSDictionary*)datas
{
    ...
    NSUInteger intWidthScreen = 130;
    NSUInteger heightImg = 130; //‡CGRectGetHeight(metUserFBView.frame);
    ...
    UIImageView *metUserFBImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, screenWidth, CGRectGetHeight(metUserFBView.frame))];
    [metUserFBImgView setImageWithURL:[NSURL URLWithString:metUserFBImgURL] placeholderImage:nil];
    metUserFBImgView.contentMode = UIViewContentModeScaleAspectFill;
    metUserFBImgView.tag = 5;
    metUserFBImgView.clipsToBounds = YES;
    ...
    visualEffectView.frame = CGRectMake(0, 0, CGRectGetWidth(metUserFBView.frame), CGRectGetHeight(metUserFBView.frame));
...
- (UIView*) displayProfileHeader
{
    ...
    NSUInteger maxWidthLastEntryDiscoverLabel = (int)screenWidth - ((int)CGRectGetMinX(labelsContainer.frame) + 20);
...
//    userSelectionTableView.tableHeaderView = metUserFBView;
    ...
    UIView *metUserFBViewLastView = [[profileHeaderView subviews] lastObject];
    
    UIView *statsContainer = [self displayMetUserStats];
    statsContainer.frame = CGRectMake(0, CGRectGetMinY(metUserFBViewLastView.frame) - CGRectGetHeight(statsContainer.frame) - 1,
...
//    [metUserFBView addSubview:statsContainer];
    ...
    float widthViews = screenWidth * 0.246875;
    for (int i = 0; i < [[self.metUserLikesDictRef filterKeysForNullObj] count]; i++) {
        ...
        NSString *nbElementsForCat = [[NSNumber numberWithInteger:[[self.metUserLikesDictRef objectForKey:[[[self.metUserLikesDictRef filterKeysForNullObj]  sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)]objectAtIndex:i]] count]] stringValue];
        statCount.text = nbElementsForCat;
        [statCount sizeToFit];
    ...
    statsContainer.frame = CGRectMake(0, 0, screenWidth, CGRectGetMaxY(statsContainerLastView.frame));
